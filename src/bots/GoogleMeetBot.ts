...
export class GoogleMeetBot extends MeetBotBase {
  ...
  private async joinMeeting({ url, name, teamId, userId, eventId, botId, pushState, uploader }: JoinParams & { pushState(state: BotStatus): void }): Promise<void> {
    ...
    if (this.earlyRecordingActive) {
      ...
      // Set up Google Meet-specific end detection for early recording
      await this.setupGoogleMeetEndDetection({
  ...
  /**
   * Sets up Google Meet-specific end detection for early recordings
   * This includes participant detection, silence detection, and modal detection
   */
  private async setupGoogleMeetEndDetection(params: {
    duration: number;
    ...
    await this.page.evaluate(
      ({ duration, inactivityLimit, userId, teamId, slightlySecretId, activateInactivityDetectionAfterMinutes }) => {
        console.log('Setting up Google Meet end detection for early recording...');
        ...
        const stopTheRecording = () => {
          console.log('Stopping early recording from Google Meet end detection...');
          try {
        ...
        let inactivityParticipantDetectionTimeout: any;
        let inactivitySilenceDetectionTimeout: any;
        let isOnValidGoogleMeetPageInterval: any;
        let dismissModalsInterval: any;
        let lastDimissError: any;

        // Check if stream has audio tracks
        const hasAudioTracks = stream.getAudioTracks().length > 0;

        // Participant detection
        const detectLoneParticipantResilient = () => {
          const re = /^[0-9]+$/;
          let loneTest: any;
          let detectionFailures = 0;
          let loneTestDetectionActive = true;
          const maxDetectionFailures = 10;
          ...
          function getContributorsCount(): number | undefined {
            ...
            try {
              ...
              console.log('Error getting contributors count');
            }
          ...
          function retryWithBackoff(): void {
            loneTest = setTimeout(function check() {
              if (!loneTestDetectionActive) {
                if (loneTest) clearTimeout(loneTest);
                return;
              }
              try {
                const contributors = getContributorsCount();
                if (typeof contributors === 'undefined') {
                  detectionFailures++;
                  console.warn('Meet participant detection failed, retrying. Failure count:', detectionFailures);
                  if (detectionFailures >= maxDetectionFailures) {
                    console.log('Persistent detection failures - stopping detection');
                    loneTestDetectionActive = false;
                  }
                  retryWithBackoff();
                  return;
            ...
            const monitorSilence = () => {
              try {
                ...
                if (audioActivity < silenceThreshold) {
                  silenceDuration += 100;
                  if (silenceDuration >= inactivityLimit) {
                    console.warn('Detected silence in Google Meet, ending recording');
                    console.log('Silence stats - Avg:', (audioActivitySum / totalChecks).toFixed(2), 'Checks:', totalChecks);
                    monitor = false;
                    stopTheRecording();
                  }
                } else {
          ...
          dismissModalsInterval = setInterval(() => {
            try {
              ...
              const dismissButtons = Array.from(buttons).filter((button) =>
                button?.offsetParent !== null && button?.innerText?.includes('Got it')
              );
              if (dismissButtons.length > 0) {
              ...
              dismissModalErrorCount += 1;
              if (dismissModalErrorCount > maxDismissModalErrorCount) {
                console.error('Failed to dismiss modals too many times, stopping...');
                clearInterval(dismissModalsInterval);
              }
            }
          }, 2000);
        ...
        const detectMeetingIsOnAValidPage = () => {
          isOnValidGoogleMeetPageInterval = setInterval(() => {
            try {
              const currentUrl = window.location.href;
              if (!currentUrl.includes('meet.google.com')) {
                console.warn('No longer on Google Meet page:', currentUrl);
                stopTheRecording();
                return;
              }
              ...
              const currentBodyText = document.body.innerText;
              if (currentBodyText.includes('You\'ve been removed from the meeting')) {
                console.warn('Bot was removed from the meeting');
                stopTheRecording();
                return;
              }

              if (currentBodyText.includes('No one responded to your request to join the call')) {
                console.warn('Bot was not admitted to the meeting');
                stopTheRecording();
                return;
              }
        ...
        inactivityParticipantDetectionTimeout = setTimeout(() => {
          console.log('Activating participant detection...');
          detectLoneParticipantResilient();
        }, activateInactivityDetectionAfterMinutes * 60 * 1000);

        inactivitySilenceDetectionTimeout = setTimeout(() => {
          console.log('Activating silence detection...');
          detectIncrediblySilentMeeting();
        }, activateInactivityDetectionAfterMinutes * 60 * 1000);

        // Start modal dismissal and page validity checks immediately
        detectModalsAndDismiss();
        detectMeetingIsOnAValidPage();

        console.log('Google Meet end detection configured for early recording');
      },
      { duration, inactivityLimit, userId, teamId, slightlySecretId, activateInactivityDetectionAfterMinutes }
    );

    this._logger.info('Google Meet end detection configured');
  }
}
