...
def main():
    parser = argparse.ArgumentParser(
        description="Seed Firebase emulator with test data"
    )
    parser.add_argument(
        "--org", default="advisewell", help="Organization ID to copy from"
    )
    parser.add_argument(
        "--limit", type=int, default=10, help="Number of meetings to copy"
    )
    parser.add_argument(
        "--days-ahead", type=int, default=1, help="Copy meetings within N days"
    )
    parser.add_argument(
        "--emulator-host", default="localhost:8080", help="Firestore emulator host"
    )
    parser.add_argument(
        "--prod-project", default="aw-development-7226", help="Production GCP project"
    )
    ...
    meetings_ref = (
        prod_db.collection("organizations").document(args.org).collection("meetings")
    )
    query = (
        meetings_ref.where("start", ">=", now)
        .where("start", "<=", cutoff)
        .limit(args.limit)
    )
    ...
    for meeting_doc in meetings:
        ...
        emulator_db.collection("organizations").document(args.org).collection(
            "meetings"
        ).document(meeting_id).set(meeting_data)
