...
def test_recurring_meeting_requeue_from_complete():
    ...
    print(f"✅ Day 1: Session completed (status={controller.sessions[session_id]['status']})")
    ...
    print(f"✅ Day 2: Session re-queued (status={controller.sessions[session_id]['status']})")
    print(f"✅ Previous status preserved: {controller.sessions[session_id].get('previous_status')}")
...
def test_recurring_meeting_requeue_from_failed():
    ...
    print(f"✅ Day 1: Session failed (status={controller.sessions[session_id]['status']})")
...
def test_session_not_requeued_while_processing():
    ...
    assert controller.sessions[session_id]["status"] == "processing", "Should still be processing"
