...
class TestMultiAttendeeConsolidation:
    ...
    def test_url_normalization_variants(self, org_id, users):
        ...
        ), "All URL variants should produce same session ID"
    ...
    def test_consolidate_to_canonical_meeting(self, org_id, meeting_url, users):
        ...
        assert (
            canonical["user_id"] == users[0]["user_id"]
        ), "First user becomes canonical owner"
        assert all(s["user_id"] in [u["user_id"] for u in users] for s in subscribers)

        print(
            f"✓ Consolidated to canonical meeting with {len(subscribers)} subscribers:"
        )
    ...
    def test_single_session_created(self, org_id, meeting_url, users):
        ...
        print(
            f"✓ Single session created with {len(session['subscribers'])} subscribers"
        )
...
class TestEndToEndFlow:
    ...
    async def test_full_multi_user_flow(self):
        ...
        duplicates = [
            ...
            if normalize_meeting_url(d["meeting_url"]) == normalized_url
        ]
        ...
        print(
            f"  ✅ Created single session with {len(session['subscribers'])} subscribers"
        )
