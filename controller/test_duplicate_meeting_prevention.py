...
def test_normalize_url():
    ...
    # URL decoding
    assert norm("https://teams.microsoft.com/meet/123%3fabc") == \
    ...
    assert norm("https://meet.google.com/abc-def-ghi/") == \
           "https://meet.google.com/abc-def-ghi"
    
    # Fragment removal
    assert norm("https://zoom.us/j/123#stuff") == \
           "https://zoom.us/j/123"
...
def test_extract_meeting_key():
    ...
    # Teams /meet/ format
    assert extract("https://teams.microsoft.com/meet/4393898968980?p=5RBY7tVvhbYL0u8L6J") == "teams:4393898968980"
    
    # Teams meetup-join format
    assert extract("https://teams.microsoft.com/l/meetup-join/19%3ameeting_MzljY123/0?context=...") == "teams:meeting_MzljY123"
...
def test_find_duplicate_meetings():
    ...
    assert len(teams_123) == 2, f"Expected 2 duplicates for teams:123, got {len(teams_123)}"
    
    teams_456 = by_key.get("teams:456", [])
    assert len(teams_456) == 1, f"Expected 1 meeting for teams:456, got {len(teams_456)}"
...
def test_choose_canonical_meeting():
    ...
    meetings = [
        MockMeeting("newer_no_bot", {
            "created_at": now,
            "bot_instance_id": None,
        }),
        MockMeeting("older_with_bot", {
            "created_at": now - timedelta(minutes=1),
            "bot_instance_id": "bot123",
        }),
    ]
    
    canonical, duplicates = DuplicateMeetingPrevention.choose_canonical_meeting(meetings)
    assert canonical.id == "older_with_bot", f"Expected older_with_bot, got {canonical.id}"
...
def test_end_to_end_deduplication():
    ...
    meetings = [
        MockMeeting("H8iOn0oau28jqyoDW3MX", {
        ...
        MockMeeting("rVvvmcMwh87IayoHdTG6", {
            "organization_id": "advisewell",
            "join_url": "https://teams.microsoft.com/meet/4393898968980?p=5RBY7tVvhbYL0u8L6J",
    ...
    # Choose canonical
    canonical, to_merge = DuplicateMeetingPrevention.choose_canonical_meeting(duplicates)
    
    # Both have bot_instance_id, so should choose the older one
    assert canonical.id == "H8iOn0oau28jqyoDW3MX", \
